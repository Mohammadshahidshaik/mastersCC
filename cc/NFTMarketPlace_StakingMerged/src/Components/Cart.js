...
const Cart = (props) => {
  ...
  const handler1 = async(e,price) => {
    ...
    try{
      const amount = { value: ethers.BigNumber.from("1000")}; 
      const productIDs = Object.keys(props.items);
      const transaction = await props.contract.BuyProductsERTN(productIDs,amount);
      Setpaystatus("Please wait ...");
      await transaction.wait();
      props.setItem({});
  ...
  <h3>MY CART</h3>
  <div className="inner">
    {props.items && Object.entries(props.items).map(([ProductID,[nftname,imageUrl,price,priceERTN]], index) => (
  ...
  <center><p><b>Total Cost : {props.TotalPayment} Wei or {props.TotalPayment1} ERTN</b></p></center>

  <center><button onClick={(e)=>{handler(e,props.TotalPayment)}} class="btn btn-dark">PAY WITH ETH</button></center><br/>
  <center><button onClick={(e)=>{handler1(e,props.TotalPayment1)}} class="btn btn-dark">PAY WITH ERTN</button></center><br/>
  <center><span>{paystaus}</span></center>
